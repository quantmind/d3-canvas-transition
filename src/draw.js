...
export function draw(node, point) {
  ...
  if (drawer === false) return;
    ...
    if (attrs["$mix-blend-mode"])
      ctx.globalCompositeOperation = attrs["$mix-blend-mode"];
    transform(node);
    //
    // Stroke
    stroke = node.getValue("stroke");
    if (stroke === "none") stroke = false;
    else {
      stroke = getColor(
        node,
        node.getValue("stroke"),
        node.getValue("stroke-opacity")
      );
